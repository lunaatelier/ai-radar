...
const Tag = ({ name, onTag, isField }) => (
  <button onClick={(e) => { e.stopPropagation(); onTag(name); }}
    ...
    className="rounded-full px-3 py-1.5 md:py-0.5 text-[13px] md:text-xs font-semibold hover:opacity-70 transition-opacity inline-flex items-center gap-1.5">
...
const StarBtn = ({ on, onClick, size = 19 }) => (
  <button onClick={(e) => { e.stopPropagation(); onClick(); }}
    ...
    className="leading-none transition-transform hover:scale-110 shrink-0 p-2 -m-2">
...
const SourceLink = ({ url }) => url ? (
  <a href={url} target="_blank" rel="noreferrer" style={{ color: "#2D66E0" }}
    className="inline-flex items-center gap-1 text-[13px] md:text-xs font-bold hover:opacity-70 py-1 md:py-0"
...
const MetaLine = ({ source, publishedAt, url }) => (
  <div style={{ borderTop: `1px solid ${T.line}` }} className="pt-2.5 flex items-center justify-between">
    <span style={{ color: T.faint }} className="text-[13px] md:text-xs font-medium">
    ...
    <li>
      <button onClick={onExpand} className="w-full flex items-center justify-between px-4 md:px-5 py-3.5 md:py-3 gap-3 text-left">
        <span className="flex items-center gap-2.5 min-w-0">
          ...
          <span style={{ color: T.ink }} className="text-[15px] md:text-sm font-semibold truncate">{item.title}</span>
          {isNewItem(item.publishedAt) && <NewDot />}
        </span>
        <span className="flex items-center gap-3 shrink-0">
          <span style={{ color: T.faint }} className="text-xs hidden md:inline">{item.source}</span>
          <StarBtn on={saved} onClick={toggle} size={17} />
      ...
      {expanded && (
        <div className="px-4 md:px-5 pb-4 pl-9 md:pl-10 flex flex-col gap-3">
          <ul className="flex flex-col gap-1.5">
            {item.lines.map((l, i) => (
              <li key={i} style={{ color: T.sub }} className="text-[15px] md:text-sm leading-relaxed flex gap-2">
  ...
  return (
    <article style={{ background: T.surface, border: `1px solid ${T.line}` }}
      ...
      <h3 style={{ color: T.ink, letterSpacing: "-0.02em" }}
        className={`font-extrabold leading-snug ${hero ? "text-lg md:text-xl" : "text-[17px] md:text-lg"}`}>
        {item.title}
      </h3>
      <ul className="flex flex-col gap-1.5">
        {item.lines.map((l, i) => (
...
const SectionTitle = ({ children, note }) => (
  <h2 style={{ color: T.ink }} className="text-[15px] md:text-sm font-extrabold tracking-wide mb-3">
      ...
      <div style={{ background: T.surface, border: `1px solid ${T.line}` }}
        ...
        <div className="flex items-center justify-between mb-3">
          <button disabled={mi <= 0} onClick={() => setYm(months[mi - 1])} aria-label="이전 달"
            style={{ color: T.faint, opacity: mi <= 0 ? 0.3 : 1 }} className="p-2 -m-2"><Icon.chevronLeft /></button>
          ...
          <button disabled={mi >= months.length - 1} onClick={() => setYm(months[mi + 1])} aria-label="다음 달"
            style={{ color: T.faint, opacity: mi >= months.length - 1 ? 0.3 : 1 }} className="p-2 -m-2"><Icon.chevronRight /></button>
        </div>
    ...
    <div style={{ background: T.bg, fontFamily: `'Pretendard','Apple SD Gothic Neo','Noto Sans KR',sans-serif`, minHeight: "100vh" }}>
      ...
      <div className="mx-auto max-w-5xl px-4 md:px-8 pb-16">
          ...
          <nav style={{ background: T.surface, border: `1px solid ${T.line}` }} className="flex rounded-full p-1 text-sm font-bold">
            ...
            <button onClick={() => { setView("saved"); setTag(null); }}
              ...
              <span style={{ background: view === "saved" ? "rgba(255,255,255,.2)" : T.line, color: view === "saved" ? "#fff" : T.sub }}
                className="rounded-full px-1.5 text-[11px] font-extrabold">{savedItems.length}</span>
            </button>
          </nav>
        ...
        {view === "settings" ? (
          <section className="pt-2">
                ...
                <div className="flex gap-2 flex-wrap mb-4">
                  ...
                  {fields.map((f) => (
                    <span key={f} style={{ background: T.fieldTint, color: T.field }}
                      ...
                      #{f}
                      <button onClick={() => toggleField(f)} className="hover:opacity-60 inline-flex p-1 -m-1" aria-label={`${f} 제거`}><Icon.x /></button>
                    </span>
                  ))}
                </div>
                ...
                <div className="flex gap-1.5 flex-wrap">
                  {allTags.filter((t) => !fields.includes(t)).map((t) => (
                    <button key={t} onClick={() => toggleField(t)}
                      ...
                      className="rounded-full px-3 py-1.5 md:py-0.5 text-[13px] md:text-xs font-bold hover:opacity-70 inline-flex items-center gap-1">
                      <Icon.plus />{t}</button>
                  ))}
                </div>
              </div>
            ...
            <div className="mt-8">
              ...
              <ListBox>
                ...
                <p style={{ borderTop: `1px solid ${T.line}`, color: T.faint }} className="p-4 text-[13px] md:text-xs leading-relaxed">
          ...
          /* ───────── 태그별 모아보기 ───────── */
          <section className="pt-2">
            <button onClick={() => setTag(null)} style={{ color: T.sub }} className="text-sm font-bold mb-4 py-1">← 오늘로 돌아가기</button>
            <div className="flex items-center gap-3 flex-wrap mb-1">
              ...
              <button onClick={() => toggleField(tag)}
                ...
                className="rounded-full px-3 py-1.5 text-[13px] md:text-xs font-bold inline-flex items-center gap-1">
          ...
          <>
              ...
              <div className="relative flex items-center gap-1 mb-5">
                <div style={{ border: `1px solid ${T.line}`, background: T.surface }} className="inline-flex items-center rounded-full overflow-hidden">
                  <button onClick={() => setSelDate(dates[di + 1])} disabled={di >= dates.length - 1} aria-label="이전 날짜"
                    style={{ color: di >= dates.length - 1 ? "#D3DCDA" : T.sub }} className="h-10 w-10 md:h-9 md:w-9 flex items-center justify-center">
                    <Icon.chevronLeft />
                  </button>
                  <button onClick={() => setShowCal((v) => !v)}
                    style={{ color: T.ink, borderLeft: `1px solid ${T.line}`, borderRight: `1px solid ${T.line}` }}
                    className="h-10 md:h-9 px-4 text-sm font-extrabold inline-flex items-center gap-1.5">
                    <Icon.calendar />{selDate ? dateLabel(selDate) : "…"}
                  </button>
                  <button onClick={() => setSelDate(dates[di - 1])} disabled={isLatest || di < 0} aria-label="다음 날짜"
                    style={{ color: isLatest || di < 0 ? "#D3DCDA" : T.sub }} className="h-10 w-10 md:h-9 md:w-9 flex items-center justify-center">
                    <Icon.chevronRight />
                  </button>
                </div>
                {!isLatest && dates.length > 0 && (
                  <button onClick={() => setSelDate(dates[0])} style={{ color: T.field, border: `1px solid #F3D9C6`, background: T.fieldTint }}
                    className="h-10 md:h-9 rounded-full px-3 text-[13px] md:text-xs font-bold">최신으로</button>
                )}
                {showCal && selDate && (
              ...
              {loadState === "loading" ? (
                ...
                <div style={{ background: T.surface, border: `1px solid ${T.line}` }} className="rounded-2xl p-5 md:p-6">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    <StatChip label="총 수집" value={`${data.totalCollected}건`} />
                    <StatChip label="핫이슈" value={`${top5.length}건`} />
                    {fields.map((f) => (
            ...
            {loadState === "ok" && (
              <>
                <section>
                  <SectionTitle note="가장 화제가 된 소식">{isLatest ? "오늘의 TOP 5" : "TOP 5"}</SectionTitle>
                  <div className="grid gap-3 md:grid-cols-2">
                    {top5[0] && (
                ...
                <section className="mt-10">
                  <SectionTitle note="관심 분야로 이미 올라간 글은 제외돼요">그 외 소식</SectionTitle>
                        ...
                        <ListBox key={key}>
                          <button onClick={() => setOpenCat(isOpen ? null : key)}
                            ...
                            <span className="flex items-center gap-2.5">
                              ...
                              <span style={{ color: T.ink }} className="font-bold text-[15px] md:text-sm">{c.label}</span>
                    ...
                    <div className="grid gap-3 md:grid-cols-3">
                      {Object.entries(tracker).map(([name, list]) => (
                        <div key={name} style={{ background: T.surface, border: `1px solid ${T.line}` }} className="rounded-2xl p-5">
                          <div style={{ color: T.ink }} className="font-extrabold text-[15px] md:text-sm mb-3">{name}</div>
                          {list.length === 0 ? (
                            <p style={{ color: T.faint }} className="text-[13px] md:text-xs">아직 수집된 업데이트가 없어요</p>
                          ) : (
                            <ul className="flex flex-col gap-2.5">
                              {list.map(([d, txt], i) => (
                                <li key={i} className="flex gap-3 text-[15px] md:text-sm">
